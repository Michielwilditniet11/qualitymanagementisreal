 ...
 *
 * The printed report is built by string concatenation and handed to
 * document.write, so every value that came from an imported file has to be
 * escaped here: a contract named `<img src=x onerror=...>` would otherwise
 * execute in a window same-origin with the app, where it can read the stored
 * register. Numbers we compute ourselves are safe, but anything off the
 * spreadsheet is not.
 */
...
${topRisks.map(s => `<tr><td>${esc(s.name)}</td><td>${esc(s.type)}</td><td><span class="badge ${s.healthScore >= 55 ? 'b-warn' : 'b-bad'}">${s.healthScore}</span></td><td>${fmtMoney(s.totalSpend)}</td><td>${s.contractCount}</td></tr>`).join('')}
</tbody></table>

<h2>Department overview</h2>
${byDepartment.map(d => `
<h3 style="margin:12px 0 4px;font-size:13px">${esc(d.name)} <span class="badge ${d.healthScore >= 80 ? 'b-ok' : d.healthScore >= 55 ? 'b-warn' : 'b-bad'}">${d.healthScore}</span></h3>
<p style="color:#666;font-size:11px">${d.contractCount} contracts · ${fmtMoney(d.totalSpend)} spend · top supplier: ${esc(d.topSupplier?.name ?? 'n/a')} (${Math.round(d.supplierConcentration * 100)}%)</p>
`).join('')}
...
${upcomingRenewals.map(c => `<tr><td>${c.endDate!.toISOString().slice(0, 10)}</td><td>${esc(c.name)}</td><td>${esc(c.supplier)}</td><td>${esc(c.department)}</td><td>${fmtMoney(c.annualValue ?? 0)}</td><td>${esc(c.owner ?? '—')}</td></tr>`).join('')}
</tbody></table>
